...
public class UIHighScoresTable : BaseGameObject
{
	...
	{
        float score = GM.GameData.PlayerData.Score();
        ...
        if(itemsAll.Where(x => x.id > your.id).Count() >= 1)
		{
            ...
            Low1.Score.SetText(low1Data.score.ToString());
		}
        else
		{
            Low1.Place.SetText("-----");
        ...
        if(itemsAll.Where(x => x.id > your.id).Count() >= 2)
		{
            ...
            Low2.Score.SetText(low2Data.score.ToString());
		}
        else
		{
            Low2.Place.SetText("-----");
