...
namespace MyColorHierarchyEditor
{
	...
	{
		private static ColorHierarchySettings settings;
		private const string PATH_TO_SETTINGS = "Assets/Plugins/MyColorHierarchy/Settings.asset";
		...
		private static class Strings
		{
			...
			public const string WINDOW_TITLE = "Color Hierarchy";
			public const string LOGO_TEXT = "Color Hierarchy";
		...
		//[MenuItem("GameObject/Color Hierarchy", false, 10)]
		private static void CreateHeaderWithMenu(MenuCommand menuCommand)
		{
			...
			headerObject.AddComponent<ColorHierarchy>();
			Undo.RegisterCreatedObjectUndo(headerObject, "Create Header");
			EditorApplication.RepaintHierarchyWindow();
		}
		
		//[MenuItem("Tools/Color Hierarchy %H")]
		...
		private static void DrawPresetCreator()
		{
			...
			if (loadPresetFoldout) {
				headerPresetToLoad = EditorGUILayout.ObjectField(
					...
					headerPresetToLoad, typeof(ColorHierarchyPreset), false);
				if (GUILayout.Button(Strings.LOAD_PRESET_BUTTON_LABEL, GUILayout.MinHeight(30f))) {
					CreateHeadersFromPreset((ColorHierarchyPreset)headerPresetToLoad);
				}
			}
		...
		private static void CreateHeader(HeaderSettings headerSettings)
		{
			...
			var headerComponentSettings = headerObject.AddComponent<ColorHierarchy>().headerSettings;
		...
		private static void CreateHeadersFromPreset(ColorHierarchyPreset presetFile)
		{
			if (presetFile == null) {
		...
		private static void CreatePresetFile()
		{
			...
			var asset = CreateInstance<ColorHierarchyPreset>();
			AssetDatabase.CreateAsset(asset, path);
			AssetDatabase.SaveAssets();

			var presetAsset = (ColorHierarchyPreset)EditorGUIUtility.Load(path);
			presetAsset.coloredHeaderPreset.Clear();

			var headerComponents = FindObjectsOfType<ColorHierarchy>();
