...
namespace MyColorHierarchyEditor
{
	public static class EditorHelper
	{
		...
		public static ColorHierarchySettings LoadSettings(string path)
		{
			var settingsObject = (ColorHierarchySettings)EditorGUIUtility.Load(path);
			if (settingsObject != null) {
				return settingsObject;
			}

			var guids = AssetDatabase.FindAssets("t:" + nameof(ColorHierarchySettings));
			if (guids.Length > 0) {
				return AssetDatabase.LoadAssetAtPath<ColorHierarchySettings>(AssetDatabase.GUIDToAssetPath(guids[0]));
			}
		...
		{
			var settingsObject = ScriptableObject.CreateInstance<ColorHierarchySettings>();
		...
		{
			var headerComponents = Object.FindObjectsOfType<ColorHierarchy>();
